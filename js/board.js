...
export const drawBoard = () => {
  ...
  for(let row = 0; row < getCurrentLayout("boardHeight"); row++) {
    ...
    for(let column = 0; column < getCurrentLayout("boardWidth"); column++) {
      ...
      if(getCurrentLayout("perf") == true) {
        let pad = new Path.Circle(new Point(holeCoords.x, holeCoords.y), globalSettings.getPadDiameter() / 2);
        pad.fillColor = globalSettings.trackColour;
        pad.strokeWidth = 1;
        pad.strokeColor = "grey";
        stripboardGroup.addChild(pad);
      }
